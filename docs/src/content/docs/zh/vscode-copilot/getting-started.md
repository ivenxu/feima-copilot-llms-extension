...
- 访问 GitHub Copilot。按照步骤[在 VS Code 中设置 GitHub Copilot](/docs/copilot/setup)。
...
3. 开始输入以下内容，输入过程中 VS Code 会提供内联建议（_幽灵文字_）：
   ...
   由于大型语言模型(Language Model)具有[不确定性](/docs/copilot/concepts/language-models#_key-characteristics)，你看到的建议可能有所不同。
...
- [GitHub Copilot 工作原理](/docs/copilot/concepts/overview)：Copilot 功能背后的核心概念、术语和架构
- [智能体(Agent)教程](/docs/copilot/agents/agents-tutorial)：使用不同智能体(Agent)类型的实践教程
- [AI 功能速查表](/docs/copilot/reference/copilot-vscode-features)：VS Code 中所有 GitHub Copilot 功能的快速参考
- [聊天文档](/docs/copilot/chat/copilot-chat)：深入了解 VS Code 中的自主编码
- [自定义指南](/docs/copilot/customization/overview)：高级个性化技巧
- [MCP 工具](/docs/copilot/customization/mcp-servers)：使用外部 API 和服务扩展智能体(Agent)
