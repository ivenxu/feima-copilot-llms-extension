...
有关 Plan 智能体(Agent)如何适应整体智能体(Agent)架构的背景知识，请参阅[智能体(Agent)概念](/vscode-copilot/agents/overview#_planning)。
...
5. 计划确定后，选择开始实施，或在编辑器中打开规划提示词(Prompt)以进一步审查。
   ...
   要实施计划，可以在同一会话中继续，也可以启动新的 [Copilot CLI 会话](/vscode-copilot/agents/copilot-cli)在后台实施计划。
...
- **创建自定义规划智能体(Agent)。** 定义一个[自定义智能体(Custom Agent)](/vscode-copilot/customization/custom-agents)，包含你规划过程的特定说明，例如强制执行架构指南或要求特定的规划交付物。
...
- [VS Code 智能体(Agent)中的记忆](https://code.visualstudio.com/docs/copilot/agents/memory)
- [为智能体(Agent)配置工具](https://code.visualstudio.com/docs/copilot/agents/agent-tools)
