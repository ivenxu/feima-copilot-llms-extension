...
无论智能体(Agent)在哪里运行，你都可以从聊天视图中的统一[会话列表](/docs/copilot/chat/chat-sessions#_sessions-list)中监控和交互所有会话。如果你更倾向于通过提示词(Prompt)工作，[Agents 应用](/docs/copilot/agents-app)提供了专用界面，以会话和聊天为核心体验，并内置所有 AI 自定义内容的访问入口。
...
- [**本地（Local）**](/docs/copilot/agents/local-agents)：使用 VS Code 智能体(Agent)循环在编辑器中交互式运行，可完全访问工作区、工具和模型。
- [**Copilot CLI**](/docs/copilot/agents/copilot-cli)：使用 Copilot CLI 在本机后台运行，可选择使用 Git worktree 进行隔离。
- [**云端（Cloud）**](/docs/copilot/agents/cloud-agents)：使用 GitHub Copilot 远程运行，并与 GitHub 拉取请求(Pull Request)集成以进行团队协作。
- [**第三方（Third-party）**](/docs/copilot/agents/third-party-agents)：使用 Anthropic 和 OpenAI 的第三方智能体(Third-party Agent) SDK，既可在本机运行也可在云端运行。
...
| 我想要... | 使用 |
|-----------|------|
| 交互式地头脑风暴、探索或迭代一个想法 | [本地智能体(Local Agent)](/docs/copilot/agents/local-agents) |
| 查询我的代码库 | [本地智能体(Local Agent)](/docs/copilot/agents/local-agents)（Ask） |
| 创建结构化实施计划 | [本地智能体(Local Agent)](/docs/copilot/agents/local-agents)（Plan） |
| 修复需要编辑器上下文的问题（测试失败、lint 错误、调试输出） | [本地智能体(Local Agent)](/docs/copilot/agents/local-agents) |
| 使用集成浏览器构建和测试 Web 应用 _(实验性)_ | [本地智能体(Local Agent)](/docs/copilot/agents/local-agents)。参见[浏览器智能体(Agent)测试指南](/docs/copilot/guides/browser-agent-testing-guide)。 |
| 使用特定的 VS Code 扩展工具或 MCP 服务器 | [本地智能体(Local Agent)](/docs/copilot/agents/local-agents) |
| 在我继续工作的同时实现定义明确的任务 | [Copilot CLI](/docs/copilot/agents/copilot-cli) 或 [云端智能体(Cloud Agent)](/docs/copilot/agents/cloud-agents) |
| 探索多个变体或概念验证 | [Copilot CLI](/docs/copilot/agents/copilot-cli) 或 [云端智能体(Cloud Agent)](/docs/copilot/agents/cloud-agents) |
| 为团队审查创建 PR | [云端智能体(Cloud Agent)](/docs/copilot/agents/cloud-agents) |
| 将 GitHub Issue 分配给智能体(Agent) | [云端智能体(Cloud Agent)](/docs/copilot/agents/cloud-agents) |
| 使用特定 AI 提供商（Anthropic、OpenAI） | [第三方智能体(Third-party Agent)](/docs/copilot/agents/third-party-agents) |
...
VS Code 有三个[内置智能体(Agent)](/docs/copilot/agents/local-agents)：

- **Agent**：自主规划并跨文件实施变更、运行终端命令和调用工具。
- **Plan**：在编写任何代码之前创建结构化的逐步实施计划，计划就绪后可移交给实施智能体(Agent)。
- **Ask**：回答有关编码概念、代码库或 VS Code 本身的问题，不进行文件修改。

对于更专业的工作流程，可以创建自己的[自定义智能体(Custom Agent)](/docs/copilot/customization/custom-agents)，定义特定角色、可用工具和语言模型(Language Model)。
...
了解更多：[权限级别和 Autopilot](/docs/copilot/agents/agent-tools#_permission-levels)。
...
- [管理聊天会话](/docs/copilot/chat/chat-sessions)：创建、切换和组织你的智能体(Agent)会话(Agent Session)。
...
- [Hooks](/docs/copilot/customization/hooks)：在生命周期事件触发时执行自定义命令，用于自动化和策略执行。
- [自定义智能体(Custom Agent)](/docs/copilot/customization/custom-agents)：创建你自己的 AI 智能体(Agent)和扩展。
