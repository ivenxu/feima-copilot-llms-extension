...
VS Code 支持多种云端智能体(Cloud Agent)，例如 Copilot 云端智能体(Cloud Agent)，以及 Claude 和 Codex 等[第三方智能体(Third-party Agent)](/vscode-copilot/agents/third-party-agents)。
...
了解更多：[VS Code 中的第三方智能体(Third-party Agent)](/vscode-copilot/agents/third-party-agents)以及如何启用它们。
...
2. 选择会话类型下拉菜单，选择 **Cloud** 以继续作为云端智能体(Cloud Agent)会话(Agent Session)。
   ...
   如果你正在使用 [Plan 智能体(Agent)](/vscode-copilot/agents/planning)，可以从**开始实施**下拉菜单选择**在云端继续**，在云端智能体(Cloud Agent)会话(Agent Session)中运行计划实施。
...
- [智能体(Agent)概览](/vscode-copilot/agents/overview)：了解不同智能体(Agent)类型和移交机制
- [后台智能体(Agent)](/vscode-copilot/agents/copilot-cli)：了解基于 CLI 的自主智能体(Agent)，用于隔离开发
- [自定义智能体(Custom Agent)](/vscode-copilot/customization/custom-agents)：创建自定义智能体(Custom Agent)角色和身份
