...
VS Code 中的 GitHub Copilot 在你输入时提供 AI 驱动的内联建议(Inline Suggestion)，自动补全代码、注释、测试等内容。内联建议(Inline Suggestion)支持广泛的编程语言和框架，是 VS Code 中多种 AI 功能之一，与用于自主多文件任务的[智能体(Agent)](/docs/copilot/agents/overview)、[聊天](/docs/copilot/chat/copilot-chat)和[智能操作(Smart Action)](/docs/copilot/copilot-smart-actions)并列。
...
不同的大型语言模型(Language Model)使用不同类型的数据进行训练，可能有不同的能力和优势。了解更多：[在 VS Code 中选择不同的 AI 语言模型(Language Model)](/docs/copilot/customization/language-models)。
...
- [选择 AI 模型](/docs/copilot/customization/language-models)
