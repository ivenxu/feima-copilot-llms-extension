...
- **尽早纠偏。** 如果 AI 走向错误方向，用后续消息[引导它](/docs/copilot/chat/chat-sessions#_send-messages-while-a-request-is-running)重定向当前请求，排队后续请求，或停止并发送新提示词(Prompt)。
...
更多信息，请参阅[向聊天提示词(Prompt)添加上下文](/docs/copilot/chat/copilot-chat-context)和[配置工具](/docs/copilot/agents/agent-tools)。
...
- **使用最新模型。** 较新的模型通常具有改进的能力。VS Code 持续添加对新模型和模型版本的支持。查看[可用模型](/docs/copilot/customization/language-models)并使用最新模型。
...
- **调整推理模型的思考强度。** 使用模型选择器中的[思考强度控件](/docs/copilot/customization/language-models#_configure-thinking-effort)，对复杂任务提高强度，对简单任务降低强度。

- **使用 BYOK 获得额外控制。** 携带自己的 API 密钥可以获得更多模型选择和托管选项。

更多信息，请参阅[选择 AI 模型](/docs/copilot/customization/language-models)和 [Copilot Chat 可用模型](https://docs.github.com/en/copilot/using-github-copilot/ai-models/changing-the-ai-model-for-copilot-chat)。
...
2. **规划。** 使用 [Plan 智能体(Agent)](/docs/copilot/agents/planning)创建结构化实施计划。在执行前审查并完善计划。
3. **实施。** 切换到智能体(Agent)模式，按计划实施。包含测试或预期输出，让智能体(Agent)可以验证自己的工作。对于较长的任务，移交给[后台智能体(Agent)](/docs/copilot/agents/copilot-cli)或[云端智能体(Cloud Agent)](/docs/copilot/agents/cloud-agents)。
4. **审查。** 使用[检查点](/docs/copilot/chat/chat-checkpoints)审查进度，如果智能体(Agent)偏离轨道则回滚，或对生成的拉取请求(Pull Request)[请求 Copilot 代码审查](https://docs.github.com/en/copilot/concepts/agents/code-review)。

更多信息，请参阅[上下文工程工作流](/docs/copilot/guides/context-engineering-guide)。
...
- **使用检查点回滚。** 如果智能体(Agent)偏离轨道，使用[检查点](/docs/copilot/chat/chat-checkpoints)回滚到已知良好状态，而不是尝试修复级联错误。

- **检查安全问题。** 审查 AI 生成的代码是否存在常见漏洞，如注入缺陷、硬编码密钥或缺少输入验证。避免在提示词(Prompt)中粘贴凭据或敏感数据。

更多信息，请参阅 [GitHub Copilot 安全性](/docs/copilot/security)和 [GitHub Copilot Trust Center](https://copilot.github.trust.page/faq)。
...
- **压缩上下文。** 使用 [/compact](/docs/copilot/chat/copilot-chat-context#_context-compaction) 并提供说明，有选择地压缩上下文，只保留最相关的信息。

- **使用子智能体(Agent)进行调查。** 提示 AI 使用[子智能体(Agent)](/docs/copilot/agents/subagents)隔离地执行研究和探索，使发现不会混乱你的主要上下文。

- **选择合适的会话类型。** 对需要立即关注的当前代码快速任务使用本地会话，对可以在本地隔离运行的任务使用后台任务，或使用可以受益于团队协作的云端会话。

- **通过并行会话扩展。** 为独立任务并行运行多个会话，节省时间并保持上下文分离。可以同时在本地、后台和云端环境中运行多个会话，并通过 VS Code 中的[会话列表](/docs/copilot/chat/chat-sessions#_sessions-list)在它们之间切换。

更多信息，请参阅[会话管理](/docs/copilot/chat/chat-sessions)和[工作区索引](/docs/copilot/reference/workspace-context)。

## 使用大型代码库

Copilot 设计用于在大型、复杂和多根工作区中有效工作。使用这些实践在规模上获得最佳结果。

- **使用工作区索引。** VS Code 使用语义搜索、语言智能和 GitHub 代码搜索自动索引项目，实现深度跨文件推理。对于大型仓库，使用[远程索引](/docs/copilot/reference/workspace-context#_remote-index)在仓库及 GitHub 上相关仓库中获得快速、全面的结果。

- **使用多根工作区限定工作范围。** 对于 monorepo 或有多个服务的项目，使用[多根工作区](/docs/editing/workspaces/multi-root-workspaces)为 AI 提供清晰的边界和专注的上下文。

- **提供项目级别的说明。** 使用[自定义说明(Custom Instructions)](/docs/copilot/customization/custom-instructions)描述项目的架构、模块边界和 AI 无法从代码中推断的约定。这为 AI 提供了架构级别变更所需的上下文。
