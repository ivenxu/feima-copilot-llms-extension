...
- 安装 Visual Studio Code Insiders。[下载 VS Code Insiders](/insiders)。
- 访问 GitHub Copilot。按照[在 VS Code 中设置 GitHub Copilot](/docs/copilot/setup)的步骤登录并激活订阅。
...
3. 对于 Copilot CLI 会话，为会话选择文件夹和 worktree [隔离](/docs/copilot/agents/copilot-cli#_isolation-modes)模式。
...
如果应用程序涉及基于浏览器的行为，可以在 Agents 应用中使用[集成浏览器](/docs/debugtest/integrated-browser)。从聊天会话中选择 `localhost` 链接，在 Agents 应用内的集成浏览器中打开它。

## 延伸阅读

- [智能体(Agent)概览](/docs/copilot/agents/overview)
- [Copilot CLI](/docs/copilot/agents/copilot-cli)
- [云端智能体(Cloud Agent)](/docs/copilot/agents/cloud-agents)
