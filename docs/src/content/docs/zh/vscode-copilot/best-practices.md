...
2. **规划。** 使用 [Plan 智能体(Agent)](/vscode-copilot/agents/planning)创建结构化实施计划，审查并完善。
3. **实施。** 切换到智能体(Agent)模式，按计划实施，包含测试或预期输出。对于较长的任务，移交给后台或云端智能体(Cloud Agent)。
4. **审查。** 使用[检查点](https://code.visualstudio.com/docs/copilot/chat/chat-checkpoints)审查进度，或请求 Copilot 代码审查。
...
- **使用工作区索引。** VS Code 使用语义搜索、语言智能和 GitHub 代码搜索自动索引项目。对于大型仓库，使用[远程索引](https://code.visualstudio.com/docs/copilot/reference/workspace-context#_remote-index)获得快速、全面的结果。
