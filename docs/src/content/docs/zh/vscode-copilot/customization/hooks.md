...
可以直接在[自定义智能体(Custom Agent)](/docs/copilot/customization/custom-agents)的 YAML frontmatter 中定义 Hooks。智能体(Agent)作用域 Hooks 仅在该自定义智能体(Custom Agent)活跃时运行，与工作区或用户级别为同一事件配置的 Hooks 同时运行。
