...
[Model Context Protocol（MCP）](https://modelcontextprotocol.io/)是连接 AI 模型与外部工具和服务的开放标准。在 Visual Studio Code 中，MCP 服务器为文件操作、数据库或外部 API 等任务提供[工具](/docs/copilot/agents/agent-tools)。MCP 服务器还可以提供[资源、提示词(Prompt)和交互式应用](#_other-mcp-capabilities)。

本文介绍如何添加、配置和管理 MCP 服务器。要了解在聊天中使用工具的信息，请参阅[配合智能体(Agent)使用工具](/docs/copilot/agents/agent-tools)。
...
> **注意**
> 本地 MCP 服务器可以在你的机器上运行任意代码。仅从[受信任的来源](#_mcp-server-trust)添加服务器，并在启动前审查发布者和服务器配置。阅读[安全文档](/docs/copilot/security)了解在 VS Code 中使用 AI 的注意事项。
...
- **用户配置文件**：运行 **MCP: Open User Configuration** 命令，打开[用户配置文件](/docs/configure/profiles)文件夹中的 `mcp.json` 文件。此处配置的服务器在所有工作区中可用。

你也可以通过命令面板（⇧⌘P / Windows、Linux：Ctrl+Shift+P）运行 **MCP: Add Server** 命令，通过引导流程添加服务器，选择**工作区**或**全局**为目标。

> **重要提示**
> 避免硬编码 API 密钥等敏感信息。请使用[输入变量](/docs/copilot/reference/mcp-configuration#_input-variables-for-sensitive-data)或环境文件代替。
...
VS Code 为配置文件提供 IntelliSense。完整的配置 Schema 和字段参考，请参阅 [MCP 配置参考](/docs/copilot/reference/mcp-configuration)。
...
启用[设置同步](/docs/configure/settings-sync)后，可以跨设备同步设置和配置，包括 MCP 服务器配置：
...
- [MCP 配置参考](/docs/copilot/reference/mcp-configuration)
- [配合智能体(Agent)使用工具](/docs/copilot/agents/agent-tools)
